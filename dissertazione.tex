...
Optic-based learning provides a structured general-purpose compositional framework for gradient-based learning. However, such great versatility has a price: optics are unable to guide the architectural design of our model. 

It has been shown times and times again that a better architecture makes as much of a difference in machine learning as an algorithm with better asymptotic cost does in classical computer science. In fact, even if we ignore performance concerns, selecting the right architecture is synonymous with selecting the right inductive bias, which is essential for effective learning.

Therefore, it is of paramount importance to find a principled mathematical framework able to guide such architectural choices. Designing an architecture is almost synonymous to imposing a set of constraints: for instance, convolutional layers are notoriously translationally invariant (ADDREF).
Hence, it makes sense to inform the choice of architecture by informing the choice of constraint.

Among the various attempts at developing a principled theory of machine learning constraints, geometric deep learning (see e.g. \cite{bronstein2021geometric}) is particulary relevant.
Geometric deep learning (GDL) focuses on equivariance constraints defined with respect to group actions, and has thus seen a lot of success in context where the transformations data can be subjected to can be expressed in group-theoretical terms. Regrettably, this is not always the case, as many transformations we can subject the data to are either not invertible or even not compositional (\cite{gavranovicposition}). Moreover, as highlighted in \cite{gavranovicposition}, although GDL is effective at describing the constraints that a model should implement, it is not always clear how such constraints can be actually be implemented.  
...
As stated in \cite{bronstein2021geometric}, GDL is inspired by the \textit{Erlangen Programme}, which unified geometry around the notions of invariance at the end of the nineteenth century. Since category theory can be seen as an extension of the \textit{Programme} (as we already remarked in the introduction to this thesis), it is only natural to attempt to generalize GDL by categorical means.


Categorical deep learning is a categorical framework for deep learning which aims to subsume and generalize GDL proposing, in the words of the authors of \cite{gavranovicposition}, the theory of (co)algebras over endofunctors and monads (see \textit{Sec. \ref{sec: algebras}}) as a "theory of all architectures". Consider the following example to see how monad algebras can model group actions and how the associated homomorphisms are the sought after equivariant maps.

\begin{example}[Group action monad]
  ...
  The monad $(G \times -, \eta, \mu)$ models the structure that lies under $G$-actions. The actions themselves can be recovered by considering algebras $(S,\ast)$ for the monad. Since $\ast$ must satisfy the commutative diagrams in ADDIM, $\ast$ satisfies the equations $e \ast s = s$ and $g \ast (h \ast s) = (g \times h) \ast s$ for all $s \in S$ and for all $g \in G$.
  Finally, an $S \to T$ map which is equivariant with respect to the actions $(S,\ast)$ and $(T,\star)$ is a $(S,\ast) \to (T,\star)$ monad algebra homomorphism because the diagram in ADDIM is commutative if and only if $f(g \ast s) = g \star f(s)$ for all $s \in S$ and for all $g \in G$.
\end{example}

The previous example shows the role that monads (or endomorphisms), the associated algebras, and the associated algebra homomorphisms play in CDL: monads (or endomorphisms) provide a shared but inert structure, algebras provide actions built on such structure, and, finally, algebra homomorphisms are maps that preserve such actions. The following example, taken from (\cite{gavranovicposition}), shows how the monad algebra paradigm can be used to impose equivariance by weight sharing.
...
\textit{Ex. \ref{ex: translationaction}
} is a simple application of geometric deep learning, but the problem is presented using the formalism of categorical deep learning. In the rest of the section, we will see applications where geometric deep learning alone does not suffice.
